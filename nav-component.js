...
(function(window) {
        ...
        container.innerHTML = `
            <nav class="hub-nav-bar" id="hub-main-nav">
                ...
                <div class="hub-nav-center">
                    ...
                    <a href="./admin.html${queryParam}" class="hub-nav-link ${activePageId === 'admin' ? 'active' : ''}" onclick="handleAdminLinkClick(event, './admin.html${queryParam}')">
                        <i class="fas fa-cog"></i> <span>الإدارة</span>
                        ${!isAdmin ? '<i class="fas fa-lock lock-icon" title="يتطلب كلمة مرور المدير"></i>' : ''}
                    </a>
                </div>
                ...
                <div class="hub-nav-right">
                    <!-- Role Badge -->
                    <span class="role-badge ${isAdmin ? 'admin' : 'user'}" onclick="handleRoleBadgeClick()" title="${isAdmin ? 'صلاحيات المدير مفعلة' : 'اضغط للترقية إلى صلاحيات المدير'}">
                        <i class="fas ${isAdmin ? 'fa-shield-halved' : 'fa-user'}"></i>
                        <span>${isAdmin ? 'مدير النظام' : 'مستخدم عادي'}</span>
                    </span>
            ...
            <div id="hub-upgrade-modal" class="hub-modal-overlay">
                <div class="hub-modal-card">
                    ...
                    <h3 style="font-size:1.15rem;font-weight:800;margin-bottom:6px;color:#1e293b;" class="dark:text-white">صلاحيات المدير</h3>
                    <p style="font-size:0.8rem;color:#64748b;margin-bottom:16px;">أدخل كلمة مرور المدير للوصول إلى لوحة التحكم والإعدادات</p>
                    
                    <input type="password" id="hub-admin-pwd-input" placeholder="كلمة مرور المدير..." 
    ...
    window.executeUpgradeToAdmin = function() {
        ...
        if (!window.Hub) return;
        const res = window.Hub.auth.upgradeToAdmin(pwd);
